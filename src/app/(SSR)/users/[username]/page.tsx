...
// const getUserCasched = cache(getUser);   Use cache if you are not using the native fetch

export async function generateMetadata({params: {username}}:userNamePageProps):Promise<Metadata> {
...
export default async function userNamePage({params: {username}}:userNamePageProps) {
    const user = await getUser(username);
    ...
    return(
        <div>
            ...
            <a href={"https://usplash.com/" + user.username}>Unsplash profile</a>
        </div>
    );
}